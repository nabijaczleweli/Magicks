...
object PotionImmunityAura extends IConfigurable {
	...
	override def configure(config: Configuration) {
		blocksPerLevel = config.getFloat("Radius per aura's level", category"potion:combat:immunity aura", blocksPerLevel, .5F, Float.MaxValue, "Radius at which the immunizing elemental aura will affect other entities")
	}
}
